...
public class LogUtils {
    ...
    public static String customTagPrefix = "ffjw"; // 自定义Tag的前缀，可以是作者名
    private static final boolean isSaveLog = true; // 是否把保存日志到SD卡中
    public static final String ROOT = Environment.getExternalStorageDirectory()
            .getPath() + "/.ffjw/"; // SD卡中的根目录
    private static final String PATH_LOG_INFO = ROOT + "/com.ruixin.ffjw/";
    ...
    public static void point(String path, String tag, Throwable tr) {
        if (isSDAva()) {
            Date date = new Date();
            SimpleDateFormat dateFormat = new SimpleDateFormat("",
                    Locale.SIMPLIFIED_CHINESE);
            ...
            try {
                out = new BufferedWriter(new OutputStreamWriter(
                        new FileOutputStream(file, true)));
                out.write(time + " " + tag + " " + Log.getStackTraceString(tr) + "\r\n");
            } catch (Exception e) {
                e.printStackTrace();
            } finally {
                try {
