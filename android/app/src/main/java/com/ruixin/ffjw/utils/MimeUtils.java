...
public final class MimeUtils {
    ...
    private static final Map<String, String> extensionToFileTypeMap = new HashMap<String, String>();
    ...
    static {
        addFileType("bmp","images");
        addFileType("gif","images");
        addFileType("jpg","images");
        addFileType("jpeg","images");
        addFileType("png","images");
        addFileType("psd","images");
        addFileType("tif","images");
        addFileType("tiff","images");

        addFileType("mkv","videos");
        addFileType("mp4","videos");
        addFileType("avi","videos");
        addFileType("swf","videos");
        addFileType("wmv","videos");
        addFileType("rmvb","videos");
        addFileType("mov","videos");
        addFileType("mpg","videos");

        addFileType("mp3","audios");
        addFileType("flac","audios");
        addFileType("ape","audios");
        addFileType("wma","audios");
        addFileType("wav","audios");
        addFileType("aac","audios");
        addFileType("m4a","audios");
        addFileType("au","audios");
        addFileType("ram","audios");
        addFileType("mmf","audios");
        addFileType("ai","audios");

        addFileType("doc","docs");
        addFileType("docx","docs");
        addFileType("pdf","docs");
        addFileType("rtf","docs");
        addFileType("txt","docs");
        addFileType("csv","docs");
        addFileType("xls","docs");
        addFileType("xlsx","docs");
        addFileType("ppt","docs");
        addFileType("pptx","docs");

        addFileType("rar","compress");
        addFileType("zip","compress");
        addFileType("7z","compress");
        addFileType("gz","compress");
        addFileType("arj","compress");
        addFileType("z","compress");
//        applyFileOverrides();
    }
    ...
    private static void addFileType(String extension, String fileType) {
        //
        // if we have an existing x --> y mapping, we do not want to
        // override it with another mapping x --> ?
        // this is mostly because of the way the mime-type map below
        ...
        // added first; we do not want to overwrite it later).
        //
        if (!extensionToFileTypeMap.containsKey(extension)) {
            extensionToFileTypeMap.put(extension, fileType);
        }
    }
    ...
    private static InputStream getFileTypesPropertiesStream() {
        // Standard location?
        File f = new File(System.getProperty("java.home"), "lib" + File.separator + "file-types.properties");
        if (f.exists()) {
            try {
                return new FileInputStream(f);
            } catch (IOException ignored) {
    ...
    private static void applyFileOverrides() {
        // Get the appropriate InputStream to read overrides from, if any.
        InputStream stream = getFileTypesPropertiesStream();
        if (stream == null) {
            return;
        }
        ...
        try {
            try {
                ...
                for (Map.Entry<Object, Object> entry : overrides.entrySet()) {
                    ...
                    addFileType(extension, fileType);
                }
            } finally {
                stream.close();
            }
    ...
    public static String guessFileTypeFromExtension(String extension) {
        if (extension == null || extension.isEmpty()) {
            return null;
        }
        return extensionToFileTypeMap.get(extension);
    }
}
