...
set /p all1='change build.gradle version 3.2.1 to':
if '%all1%'=='' (
echo ************************************************** 不修改 **************************************************
) else (
    for /f "delims=" %%i in ('type "%file%"') do (
      set str=%%i
      set "str=!str:%replaced%=%all1%!"
      echo !str!>>"%file%"_tmp.txt
    )
    move "%file%"_tmp.txt "%file%"
)
...
set /p all2='change gradle-wrapper.properties version gradle-4.6-all.zip to':
if '%all2%'=='' (
echo ************************************************** 不修改 **************************************************
) else (
    set "file=%file:"=%"
