/**
 * 身份证号码验证
 * Created by ruixin on 16/8/18.
 */
import React, {Component} from "react";

export default class IDValidator extends Component {
    static cache = {};
        ...
        checkArg: function (id) {
            ...
            switch (argType) {
                case 'number':
                    ...
                    id = id.toString();
                    if (id.length > 15) {
                        this.error(this.param.error.longNumber);
                        return false;
                    }
                    break;
                case 'string':
    ...
    static isValid(id) {
        let code = this.util.checkArg(id);
        if (code === false) {
            return false;
        }
        // 查询cache
        ...
        if (!(this.util.checkAddr(addr, GB2260) && this.util.checkBirth(birth) && this.util
                .checkOrder(order))) {
        ...
        let posWeight = [];
        for (let i = 18; i > 1; i--) {
            posWeight[i] = this.util.weight(i);
        }
