    ...
    _renderRow = (data) => {
        ...
        for (let item of data) {
            ...
            if (item && item.children && item.children.length) {
                ...
                options.push(<View style={{flex: 1}}>
                    <View style={{
                        flexDirection: 'row',
                    ...
                    </View>
                    {
                        !isOpen ? null :
                            <ScrollView style={{flex: 1, marginLeft: 15}}>
                                {this._renderRow(item.children)}
                            </ScrollView>
                    }
                </View>)
            } else {
                options.push(<TouchableOpacity onPress={(e) => this._onClickLeaf({e, item})}>
                    <View style={{
                        flexDirection: 'row',
                        backgroundColor: isCurrentNode ? selectedBackgroundColor || '#FFEDCE' : backgroundColor || '#fff',
                        marginBottom: 2,
    ...
    render() {
        ...
        return (
            <View style={treeSelectStyles.container}>
                <ScrollView style={{flex: 1, marginVertical: 5, paddingHorizontal: 15}}>
